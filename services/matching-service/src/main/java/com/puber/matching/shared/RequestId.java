 ...
 * -- the interceptor writes MDC under one name, the pattern reads another, and every log line shows
 * a blank id while the tests still pass.
 *
 * <p>Copied into each service rather than shared as a library. The copies agreeing on these
 * literals is what makes the two services agree on the wire -- so this service carries {@code
 * HEADER} with no caller. It has no HTTP edge that reads a request header; the constant is here
 * because the directory is copied whole, not because something spells it.
 */
