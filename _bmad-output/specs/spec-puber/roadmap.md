...
| --- | --- | --- | --- |
| 1. Bootstrap + domain + matching | 1–7 | Core services stand up; domain model, Postgres persistence and fixtures land; **Postgres-backed** nearest-driver matching works end to end behind the `DriverLocationIndex` strategy seam the fast path swaps out in phase 2 (AD-10, AD-26), proven by deterministic test scenarios. No Kafka yet. | — |
...
| 5. Real-time + local Kubernetes | 25–32 | Drivers get the live push channel for offers and ride-state changes; the live operational dashboard ships; the Simulator is delivered as a containerized load generator (CAP-39); the full system deploys to a local Kubernetes cluster; the stress-scale target is exercised. | `v1.0` |
