...
- **FR-2:** Rider requests a ride with pickup/dropoff coordinates and a payment-method token; the fare is locked at request time and the rider receives a ride identifier immediately, while authorization proceeds in the background (FR-9). Since there are no rider accounts (FR-47), no payment method is stored — the token travels with the request, supplied by the Simulator in practice.
- **FR-3:** A rider cannot have two simultaneous active rides — a new request is rejected while one is already in progress (`REQUESTED`/`WAITING_MATCH`/`MATCHED`/`IN_PROGRESS`).
...
- **FR-9:** No driver is dispatched until the fare is authorized, and the ride's own state is what enforces it. A ride is persisted as `REQUESTED` the instant the rider commits — that state means *awaiting authorization* and nothing else — and the hold for the locked fare (FR-33) is placed asynchronously from there. When the hold lands the ride moves to `WAITING_MATCH`; if authorization is declined it moves to the terminal `PAYMENT_FAILED` state instead and no driver is ever offered it. This is the payment-side counterpart to `NO_DRIVER` — a ride that never happened, with the reason recorded.
- **FR-10:** System matches each ride to the nearest available driver within a 5km radius; unmatched rides are retried on a scheduled interval. **Matching reads only rides in `WAITING_MATCH`**, so dispatching a ride whose funds are not yet held is structurally impossible rather than merely guarded against.
...
- **FR-12:** If no driver can be found for a `WAITING_MATCH` ride within a bounded overall window, it transitions to a terminal `NO_DRIVER` state, retrying stops, and the authorization hold is released (FR-35) — the rider gets a definitive answer instead of waiting indefinitely, and is never charged.
...
- **FR-15:** Full ride state machine: `REQUESTED → WAITING_MATCH → MATCHED → IN_PROGRESS → COMPLETED`, plus the terminal states `CANCELLED`, `NO_DRIVER`, and `PAYMENT_FAILED`. `REQUESTED` is entered exactly once and never returned to — it means *awaiting authorization*, so once a hold exists the ride can never fall back to a state that would re-authorize it. Recovery from a silent driver (FR-13, FR-14) reuses states already on the path rather than introducing recovery-only ones.
- **FR-16:** Rider may cancel only while `REQUESTED`, `WAITING_MATCH`, or `MATCHED`; cancelling a `MATCHED` ride releases the driver back to available and withdraws any offer still outstanding to them. In every case any authorization hold is released (FR-35) — including one that lands *after* the cancellation, since a ride cancelled while still `REQUESTED` may have an authorization already in flight. Cancelling with a mismatched rider identity is rejected without revealing whether the ride exists.
...
- **FR-33:** Payment follows a two-phase lifecycle against Stripe sandbox: the fare is **authorized** as a hold when the ride is requested, and **captured** when the ride completes. Authorization is asynchronous — the ride waits in `REQUESTED` until the hold resolves (FR-9) — and nothing is captured for a ride that never delivers a trip.
...
- **Ride states** — `REQUESTED → WAITING_MATCH → MATCHED → IN_PROGRESS → COMPLETED`, plus three terminal states: `CANCELLED` (rider-initiated, from `REQUESTED`/`WAITING_MATCH`/`MATCHED`), `NO_DRIVER` (system-initiated, when matching gives up), and `PAYMENT_FAILED` (system-initiated, when authorization is declined and no driver is ever dispatched).
- **`REQUESTED`** — awaiting authorization, and nothing more. Entered exactly once, when the rider commits; never returned to.
- **`WAITING_MATCH`** — funds are held and the ride is in the matching pool. The only state matching reads (FR-10), and the state a stalled match falls back to (FR-13).
