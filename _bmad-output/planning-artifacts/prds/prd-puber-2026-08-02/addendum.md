...
- **payment_intents** — one per ride, provider intent id, amount, state, plus flags for a refund or a void requested before the authorization resolved
...
**The hook, if this is picked up later:** FR-37 already drives a failed capture to terminal `FAILED` with a full audit trail. A debtor feature would read that state rather than needing anything re-architected — and a riders table (also deferred) is the natural home for the flag. Nothing about the current design forecloses it.
